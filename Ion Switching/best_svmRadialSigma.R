...
for(i in 0:10){
  ...
  id= c(id,s[sample(1:length(s),8000)]) #+50*i
}
...
lda.fit=train(open_channels ~ PC1 + PC2 + PC3 + PC4 + PC5+ PC6+PC7+
              ...
              method='svmRadialSigma',
              #family="binomial", 
              tuneGrid=expand.grid(C=8,sigma=0.02555556),
              #trControl=control,
              trControl=cv2,
              #verbosity=T,
              metric="F1")
...
res2=numeric(2000000)
for(i in 1:2) {
  dp=(1+(i-1)*10000):(i*10000)
  res2[dp] = predict(lda.fit,newdata = tst[dp,])
  print(i)
}
...
write_csv(answer,paste0(path.dir,'result_svmRadialSigma C 8 sigma 0 0256 count = 8000.csv'))



save(svmRadialSigma,file="svmRadialSigma  C 8 sigma 0 0256 count = 8000")
