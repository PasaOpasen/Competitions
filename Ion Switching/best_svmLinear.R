...
svmLinear=train(open_channels ~ PC1 + PC2 + PC3 + PC4 + PC5+ PC6+PC7+
                 ...
                 #family="binomial", 
                 trControl= cv1,
                 tuneGrid=expand.grid(C=c(0.1,0.3,0.5,0.9,1,3,5,8)),
                 #verbosity=T,
                 metric="F1")
...
write_csv(answer,paste0(path.dir,'result_svmLinear C=1 n=22000.csv'))

svmLinear=lda.fit
save(svmLinear,file='svmLinear C=1 n=22000.radata')
